...
<div class="container-fluid container-banner">
    <div class="banner">
        <img src="{{ $event->banner }}" alt="" width="100%" height="400" class="banner-ikut-event">
    </div>
            ...
            <div class="col-6">
                <div class="row row-detail d-flex flex-column container-detail">
                    ...
                    <div class="col-12 d-flex align-items-center container-profile px">
                        <img src="{{ $event->user->avatar }}" alt="" class="pp-user">
                        <a href="{{ route('user.profile', ['slug' => $event->user->slug_name]) }}" style="text-decoration: none; color: #212529"><h3 class="nama-seller">{{ $event->user->name }}</h3></a>  
                    </div>
                </div>
            </div>
            <div class="col-6 d-flex justify-content-end align-items-start">
                <div class="fav-share d-flex">
                    @auth
                    @if (Auth::user()->favorited_event->where('user_id', Auth::user()->id)->where('event_id', $event->id)->first() != null)
                    <form action="{{ route('user.event.not.like', ['id' => $event->id]) }}" id="like-event" class="d-flex align-items-center">
                        <a onclick="submitButton()" class="btn-action btn-love d-flex justify-content-center align-items-center">
                    ...
                    @else
                    <form action="{{ route('user.event.like', ['id' => $event->id]) }}" id="like-event" class="d-flex align-items-center">
                        <a onclick="submitButton()" class="btn-action btn-love d-flex justify-content-center align-items-center">
                    ...
                    @endauth
                    @guest
                    <a class="btn-action btn-love d-flex justify-content-center align-items-center">
                        <i class="material-icons love">
                            favorite
                        </i>
                        Favorit
                    ...
                    <button class="btn btn-share-link d-flex align-items-center justify-content-center clipboard">
                        <i class="material-icons-outlined">
                            insert_link
                        </i>
                    </button>
                    <button class="btn btn-whatsapp d-flex align-items-center justify-content-center">
                        <a href="https://wa.me/?text={{ Request::url() }}" target="_blank">
                            <i class="fab fa-whatsapp"></i>
                        </a>
                    </button>
                    <button class="btn btn-twitter d-flex align-items-center justify-content-center">
                        <a href="https://twitter.com/intent/tweet?text=Sharing+event&url={{ Request::url() }}" target="_blank">
        ...
        <div class="container">
            <div class="row d-flex align-items-center">
                ...
                <div class="col-5 col d-flex align-items-center justify-content-end">
                    ...
                    <div class="col-6 d-flex justify-content-center">
                        <h4 class="rp">Rp<span id="subtotal" style="color: black">0</span></h4>
...
<script>
// $('.btn-love').on('click', function(event, count) {
...
function submitButton() {
    document.getElementById('like-event').submit();
}
</script>
...
const handleIncrement = (node, price) => {
    ...
    if (countTrue < {{ $event->max_ticket_user }} && countDisabled < 5) {
        ...
        subTotalText.innerText = subtotal.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.');
...
@push('styles')
    <link rel="stylesheet" href="/src/css/ikut-event.css">
@endpush
